...
pub struct TextInput {
    ...
    pub on_change: Option<Box<dyn Fn(String, &mut Window, &mut Context<TextInput>) + 'static>>,
}
...
impl TextInput {
    ...
    pub fn on_change(mut self, handler: impl Fn(String, &mut Window, &mut Context<TextInput>) + 'static) -> Self {
    ...
    fn delete_to_previous_word(&mut self, _: &DeleteToPreviousWord, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
    ...
    fn delete_to_next_word(&mut self, _: &DeleteToNextWord, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
    ...
    fn next_word_boundary(&self, offset: usize) -> usize {
        self.content
            .split_word_bound_indices()
            .find_map(|(idx, str)| (idx > offset || (idx == offset && !str.trim().is_empty())).then_some(idx + str.len()))
            .unwrap_or(self.content.len())
    }
}
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), theme.on_surface_variant())
        } else {
            (content, style.color)
        };
        ...
        let (selection, cursor) = if selected_range.is_empty() {
            let show_cursor = input.focus_handle.is_focused(window);
            (
                None,
                if show_cursor {
                    Some(fill(
...
impl Render for TextInput {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let bg_color = theme.surface_variant().opacity(0.3);
